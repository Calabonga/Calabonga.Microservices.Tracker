...
namespace Calabonga.Microservices.Tracker.Extensions
{
    ...
    public static class TrackerServiceCollectionExtensions
    {
        ...
        /// <param name="configure">Configure action for options</param>
        /// <returns>An instance of <see cref="ITrackerBuilder"/> which to be used to configure correlation ID providers and options.</returns>
        public static ITrackerBuilder AddCommunicationTracker<T>(this IServiceCollection services, Action<TrackerOptions> configure) where T : class, ITrackerIdGenerator
        {
            if (services is null)
            {
                throw new ArgumentNullException(nameof(services));
